...
function getRightPopup(marker, usePopup) {
	...
	"playgroundPopup":
		{"home": {"content": `<h1>${ ((poi.tags["name"] != undefined) ? poi.tags["name"] : "bla") }</h1><h2>${ ((poi.tags["name"] != undefined) ? "" : marker.name) }</h2>${ processContentDatabase(marker, poi, PEP_data) }`, "symbol": "🏠", "title": getText().PDV_TITLE_HOME, "active": true, "default": true},
